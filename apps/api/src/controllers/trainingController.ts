...
interface TrainingModule {
  ...
  role_id: number;
  category: string;
  content_type: 'video' | 'document' | 'interactive' | 'assessment';
  content_url?: string;
  duration: number;
  prerequisites: any;
  is_active: boolean;
...
export const getTrainingModules = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const modules = await executeQuery<any[]>(query, params);
    
    logger.info(`Retrieved ${modules.length} training modules for user ${userId}`);
    ...
    logger.error('Error retrieving training modules:', error);
    res.status(500).json({ error: 'Failed to retrieve training modules' });
  }
});

// Get training module by ID
export const getTrainingModuleById = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const userId = req.user?.id;

  try {
    const modules = await executeQuery<TrainingModule[]>(`
      SELECT tm.*, r.name as role_name
      FROM training_modules tm
      LEFT JOIN roles r ON tm.role_id = r.id
    ...
    const progress = await executeQuery<TrainingProgress[]>(`
    ...
    if (module.prerequisites) {
      try {
        ...
        if (Array.isArray(prereqIds) && prereqIds.length > 0) {
          prerequisites = await executeQuery<any[]>(`
            SELECT id, title, category FROM training_modules 
            WHERE id IN (${prereqIds.map(() => '?').join(',')})
          `, prereqIds);
        }
      } catch (e) {
        logger.warn(`Failed to parse prerequisites for module ${id}:`, e);
      }
    }

    logger.info(`Retrieved training module ${id} for user ${userId}`);
    ...
    logger.error('Error retrieving training module:', error);
    res.status(500).json({ error: 'Failed to retrieve training module' });
  }
});

// Get user's training progress
export const getTrainingProgress = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const userId = req.user?.id;
  ...
  try {
    ...
    logger.info(`Training progress retrieved for user ${userId}`);
    ...
    logger.error('Error retrieving training progress:', error);
    res.status(500).json({ error: 'Failed to retrieve training progress' });
  }
});

// Start training module
export const startTraining = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const userId = req.user?.id;
  ...
  try {
    ...
    if (module.prerequisites) {
      try {
        const prereqIds = JSON.parse(module.prerequisites);
        if (Array.isArray(prereqIds) && prereqIds.length > 0) {
          const prereqProgress = await executeQuery<any[]>(`
        ...
        logger.warn(`Failed to check prerequisites for module ${id}:`, e);
      }
    }

    // Check if progress already exists
    const existingProgress = await executeQuery<TrainingProgress[]>(`
    ...
    logger.info(`Training module ${id} started by user ${userId}`);
...
export const updateTrainingProgress = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  ...
  try {
    ...
    logger.info(`Training progress updated for module ${id} by user ${userId}`);
...
export const completeTraining = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  ...
  try {
    ...
    logger.info(`Training module ${id} completed by user ${userId}`);
...
export const createTrainingModule = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    logger.info(`Training module created with ID ${result.insertId} by user ${createdBy}`);
...
export const updateTrainingModule = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  ...
  try {
    ...
    const existingModules = await executeQuery<TrainingModule[]>(`
    ...
    // Update module with safe fallbacks
    await executeQuery(`
    ...
    logger.info(`Training module ${id} updated by user ${updatedBy}`);
...
export const deleteTrainingModule = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  ...
  try {
    ...
    if (progress[0].count > 0) {
      ...
      await executeQuery('DELETE FROM training_progress WHERE module_id = ?', [id]);
      await executeQuery('DELETE FROM training_modules WHERE id = ?', [id]);
    }

    logger.info(`Training module ${id} deleted by user ${deletedBy}`);
